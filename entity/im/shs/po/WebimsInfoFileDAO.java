...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsInfoFile entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsInfoFile
  * @author MyEclipse Persistence Tools 
 */

public class WebimsInfoFileDAO implements IWebimsInfoFileDAO {
	// property constants
	...
	*/
	public void save(WebimsInfoFile entity) {
		EntityManagerHelper.log("saving WebimsInfoFile instance", Level.INFO,
				null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsInfoFileDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsInfoFile entity) {
		EntityManagerHelper.log("deleting WebimsInfoFile instance", Level.INFO,
				null);
		try {
			entity = getEntityManager().getReference(WebimsInfoFile.class,
					entity.getId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsInfoFileDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsInfoFile update(WebimsInfoFile entity) {
		EntityManagerHelper.log("updating WebimsInfoFile instance", Level.INFO,
				null);
		try {
			WebimsInfoFile result = getEntityManager().merge(entity);
	...
	public WebimsInfoFile findById(WebimsInfoFileId id) {
		EntityManagerHelper.log("finding WebimsInfoFile instance with id: "
				+ id, Level.INFO, null);
		try {
			WebimsInfoFile instance = getEntityManager().find(
					WebimsInfoFile.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsInfoFile> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsInfoFile instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsInfoFile model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsInfoFile> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsInfoFile instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsInfoFile model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
