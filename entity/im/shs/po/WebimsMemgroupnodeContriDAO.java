...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsMemgroupnodeContri entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsMemgroupnodeContri
  * @author MyEclipse Persistence Tools 
 */

public class WebimsMemgroupnodeContriDAO implements
		IWebimsMemgroupnodeContriDAO {
	// property constants
	...
	*   WebimsMemgroupnodeContriDAO.save(entity);
	...
	*/
	public void save(WebimsMemgroupnodeContri entity) {
		EntityManagerHelper.log("saving WebimsMemgroupnodeContri instance",
				Level.INFO, null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsMemgroupnodeContriDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsMemgroupnodeContri entity) {
		EntityManagerHelper.log("deleting WebimsMemgroupnodeContri instance",
				Level.INFO, null);
		try {
			entity = getEntityManager().getReference(
					WebimsMemgroupnodeContri.class, entity.getId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsMemgroupnodeContriDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsMemgroupnodeContri update(WebimsMemgroupnodeContri entity) {
		EntityManagerHelper.log("updating WebimsMemgroupnodeContri instance",
				Level.INFO, null);
		try {
			WebimsMemgroupnodeContri result = getEntityManager().merge(entity);
	...
	public WebimsMemgroupnodeContri findById(WebimsMemgroupnodeContriId id) {
		EntityManagerHelper.log(
				"finding WebimsMemgroupnodeContri instance with id: " + id,
				Level.INFO, null);
		try {
			WebimsMemgroupnodeContri instance = getEntityManager().find(
					WebimsMemgroupnodeContri.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsMemgroupnodeContri> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsMemgroupnodeContri instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsMemgroupnodeContri model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsMemgroupnodeContri> findAll(
			final int... rowStartIdxAndCount) {
		EntityManagerHelper.log(
				"finding all WebimsMemgroupnodeContri instances", Level.INFO,
				null);
		try {
			final String queryString = "select model from WebimsMemgroupnodeContri model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
