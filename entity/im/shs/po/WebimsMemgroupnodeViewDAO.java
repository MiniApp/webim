...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsMemgroupnodeView entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsMemgroupnodeView
  * @author MyEclipse Persistence Tools 
 */

public class WebimsMemgroupnodeViewDAO implements IWebimsMemgroupnodeViewDAO {
	// property constants
	...
	*   WebimsMemgroupnodeViewDAO.save(entity);
	...
	*/
	public void save(WebimsMemgroupnodeView entity) {
		EntityManagerHelper.log("saving WebimsMemgroupnodeView instance",
				Level.INFO, null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsMemgroupnodeViewDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsMemgroupnodeView entity) {
		EntityManagerHelper.log("deleting WebimsMemgroupnodeView instance",
				Level.INFO, null);
		try {
			entity = getEntityManager().getReference(
					WebimsMemgroupnodeView.class, entity.getId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsMemgroupnodeViewDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsMemgroupnodeView update(WebimsMemgroupnodeView entity) {
		EntityManagerHelper.log("updating WebimsMemgroupnodeView instance",
				Level.INFO, null);
		try {
			WebimsMemgroupnodeView result = getEntityManager().merge(entity);
	...
	public WebimsMemgroupnodeView findById(WebimsMemgroupnodeViewId id) {
		EntityManagerHelper.log(
				"finding WebimsMemgroupnodeView instance with id: " + id,
				Level.INFO, null);
		try {
			WebimsMemgroupnodeView instance = getEntityManager().find(
					WebimsMemgroupnodeView.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsMemgroupnodeView> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsMemgroupnodeView instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsMemgroupnodeView model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsMemgroupnodeView> findAll(
			final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsMemgroupnodeView instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsMemgroupnodeView model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
