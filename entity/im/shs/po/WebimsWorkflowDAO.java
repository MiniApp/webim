...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsWorkflow entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsWorkflow
  * @author MyEclipse Persistence Tools 
 */

public class WebimsWorkflowDAO implements IWebimsWorkflowDAO {
	// property constants
	public static final String WORKFLOWGROUP_ID = "workflowgroupId";
	...
	*/
	public void save(WebimsWorkflow entity) {
		EntityManagerHelper.log("saving WebimsWorkflow instance", Level.INFO,
				null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsWorkflowDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsWorkflow entity) {
		EntityManagerHelper.log("deleting WebimsWorkflow instance", Level.INFO,
				null);
		try {
			entity = getEntityManager().getReference(WebimsWorkflow.class,
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsWorkflowDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsWorkflow update(WebimsWorkflow entity) {
		EntityManagerHelper.log("updating WebimsWorkflow instance", Level.INFO,
				null);
		try {
			WebimsWorkflow result = getEntityManager().merge(entity);
	...
	public WebimsWorkflow findById(Integer id) {
		EntityManagerHelper.log("finding WebimsWorkflow instance with id: "
				+ id, Level.INFO, null);
		try {
			WebimsWorkflow instance = getEntityManager().find(
					WebimsWorkflow.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsWorkflow> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsWorkflow instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsWorkflow model where model."
	...
	public List<WebimsWorkflow> findByWorkflowgroupId(Object workflowgroupId,
		...
		return findByProperty(WORKFLOWGROUP_ID, workflowgroupId,
				rowStartIdxAndCount);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsWorkflow> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsWorkflow instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsWorkflow model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
