...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsModelCustom entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsModelCustom
  * @author MyEclipse Persistence Tools 
 */

public class WebimsModelCustomDAO implements IWebimsModelCustomDAO {
	// property constants
	...
	*   WebimsModelCustomDAO.save(entity);
	...
	*/
	public void save(WebimsModelCustom entity) {
		EntityManagerHelper.log("saving WebimsModelCustom instance",
				Level.INFO, null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsModelCustomDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsModelCustom entity) {
		EntityManagerHelper.log("deleting WebimsModelCustom instance",
				Level.INFO, null);
		try {
			entity = getEntityManager().getReference(WebimsModelCustom.class,
					entity.getId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsModelCustomDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsModelCustom update(WebimsModelCustom entity) {
		EntityManagerHelper.log("updating WebimsModelCustom instance",
				Level.INFO, null);
		try {
			WebimsModelCustom result = getEntityManager().merge(entity);
	...
	public WebimsModelCustom findById(WebimsModelCustomId id) {
		EntityManagerHelper.log("finding WebimsModelCustom instance with id: "
				+ id, Level.INFO, null);
		try {
			WebimsModelCustom instance = getEntityManager().find(
					WebimsModelCustom.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsModelCustom> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsModelCustom instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsModelCustom model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsModelCustom> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsModelCustom instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsModelCustom model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
