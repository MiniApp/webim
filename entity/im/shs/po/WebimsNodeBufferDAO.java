...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsNodeBuffer entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsNodeBuffer
  * @author MyEclipse Persistence Tools 
 */

public class WebimsNodeBufferDAO implements IWebimsNodeBufferDAO {
	// property constants
	...
	*   WebimsNodeBufferDAO.save(entity);
	...
	*/
	public void save(WebimsNodeBuffer entity) {
		EntityManagerHelper.log("saving WebimsNodeBuffer instance", Level.INFO,
				null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsNodeBufferDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsNodeBuffer entity) {
		EntityManagerHelper.log("deleting WebimsNodeBuffer instance",
				Level.INFO, null);
		try {
			entity = getEntityManager().getReference(WebimsNodeBuffer.class,
					entity.getNodeId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsNodeBufferDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsNodeBuffer update(WebimsNodeBuffer entity) {
		EntityManagerHelper.log("updating WebimsNodeBuffer instance",
				Level.INFO, null);
		try {
			WebimsNodeBuffer result = getEntityManager().merge(entity);
	...
	public WebimsNodeBuffer findById(Integer id) {
		EntityManagerHelper.log("finding WebimsNodeBuffer instance with id: "
				+ id, Level.INFO, null);
		try {
			WebimsNodeBuffer instance = getEntityManager().find(
					WebimsNodeBuffer.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsNodeBuffer> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsNodeBuffer instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsNodeBuffer model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsNodeBuffer> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsNodeBuffer instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsNodeBuffer model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
