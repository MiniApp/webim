...
/**
 	* A data access object (DAO) providing persistence and search support for WebimsInfoTag entities.
 	 		* Transaction control of the save(), update() and delete() operations must be handled externally by senders of these methods 
 		  or must be manually added to each of these methods for data to be persisted to the JPA datastore.	
 	 * @see im.shs.po.WebimsInfoTag
  * @author MyEclipse Persistence Tools 
 */

public class WebimsInfoTagDAO implements IWebimsInfoTagDAO {
	// property constants
	...
	*/
	public void save(WebimsInfoTag entity) {
		EntityManagerHelper.log("saving WebimsInfoTag instance", Level.INFO,
				null);
		try {
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   WebimsInfoTagDAO.delete(entity);
	 *   EntityManagerHelper.commit();
	 *   entity = null;
	 * </pre>
	...
	public void delete(WebimsInfoTag entity) {
		EntityManagerHelper.log("deleting WebimsInfoTag instance", Level.INFO,
				null);
		try {
			entity = getEntityManager().getReference(WebimsInfoTag.class,
					entity.getId());
	 ...
	 * <pre>
	 *   EntityManagerHelper.beginTransaction();
	 *   entity = WebimsInfoTagDAO.update(entity);
	 *   EntityManagerHelper.commit();
	 * </pre>
	...
	public WebimsInfoTag update(WebimsInfoTag entity) {
		EntityManagerHelper.log("updating WebimsInfoTag instance", Level.INFO,
				null);
		try {
			WebimsInfoTag result = getEntityManager().merge(entity);
	...
	public WebimsInfoTag findById(WebimsInfoTagId id) {
		EntityManagerHelper.log(
				"finding WebimsInfoTag instance with id: " + id, Level.INFO,
				null);
		try {
			WebimsInfoTag instance = getEntityManager().find(
					WebimsInfoTag.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WebimsInfoTag> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding WebimsInfoTag instance with property: " + propertyName
						+ ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WebimsInfoTag model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<WebimsInfoTag> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all WebimsInfoTag instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WebimsInfoTag model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
